//Create an array of four animals called animals.
//Create a function called nameAnimals.
//Within your function, create a for loop that logs "Mommy, 
//I want to see [insert animal name here]! Waaah!"
//With your array (and - if needed - with your knowledge of parameters),
// invoke your function to ensure it is working correctly.
var animals = ['lemur','tazmanian devil', 'platypus','sloth']
function nameAnimals(animals){
    var complain =[]
    for( var i=0;i<=animals.lenth;i++){
    return("mommy I want to see "[i]"!, Waah!!");


}
}
console.log(nameAnimals(complain));
//---------------------------------------------
//Create a function that dynamically adds the following 
//animal names to the current array. You must NOT recreate the array, only add values.
function addsAnimals()






